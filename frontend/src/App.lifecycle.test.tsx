...
function mockHomepage() {
  ...
  const syncItems = vi.fn().mockResolvedValue(
    response({
      synced_at: '2026-09-12T10:00:00',
      warnings: [],
    }),
  )
  vi.stubGlobal(
...
describe('App background lifecycle', () => {
  ...
  it.each(['en', 'zh'] as const)(
    'labels an offline watchlist with its saved time and clears the label on recovery in %s',
    async (language) => {
      ...
      window.localStorage.setItem(
        'investment-board-watchlist-cache',
        JSON.stringify({
          items: watchlist,
          timestamp: savedAt,
        }),
      ...
      const label =
        language === 'en' ? /Cached watchlist, saved/ : /自选股本地缓存，保存于/
      ...
      expect(
        JSON.parse(
          window.localStorage.getItem('investment-board-watchlist-cache')!,
        ).timestamp,
      ).toBe(savedAt)
      ...
      expect(
        screen.getByText(language === 'en' ? /Last refresh/ : /上次刷新/),
      ).toBeInTheDocument()
    },
  )
  ...
  it('keeps the three-minute sync schedule through successful one-minute refreshes', async () => {
    ...
    for (let minute = 1; minute <= 6; minute += 1) {
      ...
      await act(async () => {
        pendingRefresh.resolve(response(watchlist))
      })
  ...
  it('does not overlap sync requests and resumes on the next interval after completion', async () => {
    ...
    await act(async () => {
      pendingSync.resolve(
        response({ synced_at: '2026-09-12T10:00:00', warnings: [] }),
      )
    })
    await act(async () => {
      await vi.advanceTimersByTimeAsync(180_000)
  ...
  it('preserves persisted settings on a StrictMode mount and saves edits across a remount', async () => {
    ...
    expect(screen.getByLabelText('Watchlist page shell')).toHaveClass(
      'dashboard-shell--zh',
...
it('keeps the full app usable when preference storage is denied', async () => {
  vi.stubGlobal(
    'fetch',
    vi.fn((url: string) =>
      Promise.resolve(
        response(
          url === '/api/watchlist/items'
            ? []
            : { indexes: [], macro: [], updated_at: null, warnings: [] },
        ),
      ),
    ),
  )
...
it('retains valid saved preferences when writes are denied and permits session edits', async () => {
  const saved = {
    ...DEFAULT_HOMEPAGE_SETTINGS,
    language: 'zh',
    density: 'comfortable',
  ...
  try {
    ...
    expect(screen.getByLabelText('密度')).toHaveValue('compact')
    expect(
      JSON.parse(window.localStorage.getItem(HOMEPAGE_SETTINGS_STORAGE_KEY)!),
    ).toEqual(saved)
  } finally {
