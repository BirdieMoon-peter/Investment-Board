...
const initial = {
  ...
  mutation_token: 'fixture-private-token',
}
...
describe('Homepage AI settings integration', () => {
  it('loads only when the AI settings tab is visited, preserves language edits, and never puts AI credentials in browser storage', async () => {
    const settings = {
      ...DEFAULT_HOMEPAGE_SETTINGS,
    ...
    const fetch = vi.fn((url: string, options?: RequestInit) => {
      ...
      if (url === '/api/homepage/overview')
        return Promise.resolve(
          response({ indexes: [], macro: [], updated_at: null, warnings: [] }),
        )
      if (url === '/api/ai/settings') {
        if (options?.method === 'PUT')
          return Promise.resolve(response({ ...initial, source: 'web' }))
    ...
    click(screen.getByRole('tab', { name: 'AI service configuration' }))
    ...
    click(screen.getByRole('tab', { name: 'Interface and refresh' }))
    fireEvent.change(screen.getByLabelText('Language'), {
      target: { value: 'zh' },
    })
    expect(document.documentElement).toHaveAttribute('lang', 'zh-CN')
    click(screen.getByRole('tab', { name: 'AI 服务配置' }))
    expect(screen.getByLabelText('模型名称')).toHaveValue('unsaved-model')
    expect(screen.getByLabelText('新 API 密钥')).toHaveValue(
      'fixture-private-key',
    )
    expect(
      fetch.mock.calls.filter(([url]) => url === '/api/ai/settings'),
    ).toHaveLength(1)
    click(screen.getByRole('button', { name: '保存 AI 配置' }))
    await screen.findByText('AI 配置已保存，后续 AI 请求将使用此配置。')
    const write = fetch.mock.calls.find(
      ([url, options]) =>
        url === '/api/ai/settings' && options?.method === 'PUT',
    )
    ...
    expect(write![1]!.headers).toMatchObject({
      'X-AI-Settings-Token': 'fixture-private-token',
    })
    expect(JSON.stringify(window.localStorage)).not.toMatch(
      /fixture-private|unsaved-model|fixture-model/,
    )
    expect(JSON.stringify(window.sessionStorage)).not.toMatch(
      /fixture-private|unsaved-model|fixture-model/,
    )
    expect(
      JSON.parse(window.localStorage.getItem(HOMEPAGE_SETTINGS_STORAGE_KEY)!),
    ).toMatchObject({ ...settings, language: 'zh' })
    click(screen.getByRole('button', { name: '关闭设置' }))
    expect(screen.queryByLabelText('模型名称')).not.toBeInTheDocument()
    click(await screen.findByRole('button', { name: '打开设置' }))
    click(screen.getByRole('tab', { name: 'AI 服务配置' }))
    ...
    await waitFor(() =>
      expect(
        fetch.mock.calls.filter(
          ([url, options]) =>
            url === '/api/ai/settings' && options?.method === 'GET',
        ),
      ).toHaveLength(2),
    )
  })
