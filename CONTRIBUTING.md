...
- 开始前阅读 README，确认相关接口与现有测试；功能或配置发生变化时，同步使用说明。
- 每次改动聚焦一个问题，避免将无关重构混入功能修复。
- 在提交说明中写清问题、行为变化和实际运行的验证；合并前完成代码审查与相关检查。
- 行情、新闻与 AI 接口使用可复现的测试响应。共享演示和截图使用独立的演示数据。
- 不提交 `.env.local`、密钥、个人自选股/持仓数据库、依赖目录、构建产物、Agent 指令、memory、执行计划或原始验证记录。共享文档也应去除个人账户、持仓及本机路径信息。
